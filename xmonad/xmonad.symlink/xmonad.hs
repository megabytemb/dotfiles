...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    -- launch gmrun
    , ((modMask .|. controlMask, xK_l     ), spawn "gdmflexiserver")

    -- launch dmenu
    , ((modMask,               xK_p     ), spawn "exe=`dmenu_path | ~/.dotfiles/bin/dmenu` && eval \"exec $exe\"")
...
myManageHook = composeAll
    ...
    , className =? "Transmission-gtk"   --> doShift "5:media"
...
myStartupHook = do
    setWMName "LG3D"
    -- quit pre-existing thunar daemon if it exists.
    spawn "~/.dotfiles/bin/xmonadstart"
